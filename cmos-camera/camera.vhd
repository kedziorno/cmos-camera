...
--p4 : process (camera_i_xclk) is
--  constant C_CAMERA_WIDTH : integer := 640;
--  constant C_CAMERA_HEIGHT : integer := 480;
--  variable count : integer range 0 to C_CAMERA_WIDTH * C_CAMERA_HEIGHT - 1;
--  variable vd : std_logic_vector(7 downto 0);
--begin
--  if (camera_i_rst = '0') then
--    vd := (others => '0');
--    count := 0;
--  elsif (falling_edge(camera_i_xclk)) then -- the camera sends data on the falling edge, processed to his clock
--    if (cam_vs = '1') then
--      count := 0;
--    end if;
--    if (pixel_time = '1') then
